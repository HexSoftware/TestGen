...
public class TestOverviewUI {
    ...
    private static void placeComponents(final JFrame frame, JPanel panel, final Test test, final Student student) {
        ...
        description.setBounds(325, 125, 200, 25);
        ...
        try {
				if (test.getTestParam("state").equals("scheduled") && !proctorMethods.checkStatus(test)) {
			...
			} else if (test.getTestParam("state").equals("Open") || proctorMethods.checkStatus(test)){
				  if (!test.getTestParam("state").equals("Open")) {
				  ...
				  System.out.println("Password is :" + test.getTestParam("password"));
				  if (!test.getTestParam("password").equals("null")) {
